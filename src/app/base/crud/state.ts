...
import { computed, effect, inject, linkedSignal, Service, Signal, signal, Type } from "@angular/core";
import { ConfService } from "@libs/conf/service";
import { LogService } from "@libs/log/service";
import { SignalStateService } from "@libs/signal-state/service";
import { CrudActionRecordPrimaryKeyValueType, CrudActionRecordSecondaryKeyValueType, CrudEndDrawerOnCloseType, CrudFieldOptionType, CrudFieldSwitchOptionType, CrudFormFieldInfoType, CrudListingFormattedFieldObjType, CrudSlotFieldPortalType, CrudSlotFieldsType, CrudStateFormFieldObjType, CrudStateListingDataType, CrudStateListingFieldObjType, CrudStateListOperationFieldObjType, CrudStateMutationFieldObjType, CrudStateSearchFilterFieldObjType, CrudStateViewOptionFieldObjType, CrudUniqueKeyType } from "@base/crud/type";
import { CrudActionUiLayoutEnum, CrudFieldNormalizeModeEnum, CrudFieldUiTypeEnum, CrudListingAdditionalColumnsEnum, CrudListingItemPerPageOptionEnum, CrudListOperationFieldsEnum, CrudViewOptionFieldsEnum } from "@base/crud/enum";
...
export class CrudState extends SignalStateService implements FoundationModuleStateType {
    ...
    /**
     * Root singleton. Injected here rather than CrudUrl, which is impossible:
     * CrudUrl already does inject(CrudState). UrlService has no CRUD
     * knowledge, so nothing points back.
     */
    public readonly url = inject(UrlService);

    /**
     * CRUD's URL surface. Injectable here because CrudRoute depends on
     * nothing inside CRUD — unlike CrudUrl and CrudUtility, which both
     * inject this class.
     */
    ...
    public readonly DEFAULT_LIST_OPERATION_FIELD_OBJ: CrudStateListOperationFieldObjType = {
        ...
        listing_selected_rows: {
            ...
            //         rows
            //             .map((row: any) => this.getRecordPrimaryKeyValue(row, primaryKey))
            ...
            //                     return this.getRecordPrimaryKeyValue(item, primaryKey);
            //                 }
            //
            //                 /**
            ...
            //             const rowId = this.getRecordPrimaryKeyValue(row, primaryKey);
            //             return rowId !== null &&
            //                 rowId !== undefined &&
            ...
            normalize_val: (v: any, fi: CrudFormFieldInfoType, r: Record<string, any>, mode: CrudFieldNormalizeModeEnum) => {
                ...
                const secondaryKey = this?.secondaryKey?.() ?? undefined;
                ...
                const loadedRowKeys = new Set(
                    rows
                        .map((row: any) => this.getRecordSecondaryKeyValue(row, secondaryKey))
                ...
                if (mode === CrudFieldNormalizeModeEnum.STOC) {
                    ...
                    const keys = selectedItems
                        .map((item) => {
                             ...
                             */
                            if (item && typeof item === 'object') {
                                return this.getRecordSecondaryKeyValue(item, secondaryKey);
                            }
                ...
                const selectedRows = uniqueUrlKeys
                    .map((urlKey) => rows.find((row: any) => {
                        const rowKey = this.getRecordSecondaryKeyValue(row, secondaryKey);
                        return rowKey !== null &&
                            rowKey !== undefined &&
    ...
    public readonly DEFAULT_VIEW_OPTION_FIELD_OBJ: CrudStateViewOptionFieldObjType = {
        ...
        sort_fields_and_direction: {
            label: 'CRUD.VIEW_OPTION.SORT_FIELDS_AND_DIRECTION.LABEL',
            type: CrudFieldUiTypeEnum.MULTISELECT,
            url_matrix_param: 'sfad',
            placeholder: 'CRUD.VIEW_OPTION.SORT_FIELDS_AND_DIRECTION.PLACEHOLDER',
            hint: 'CRUD.VIEW_OPTION.SORT_FIELDS_AND_DIRECTION.HINT',
            value: null,
            default: null,
            option: null, // will be filled dynamically at initialization
            validation: {
                option_range: {
                    message: 'CRUD.VIEW_OPTION.SORT_FIELDS_AND_DIRECTION.VALIDATION.OPTION_RANGE',
                    value: null
                },
            }
        },
        rbin: {
            label: 'CRUD.VIEW_OPTION.RBIN.LABEL',
            type: CrudFieldUiTypeEnum.SWITCH,
            url_matrix_param: 'rb',
            placeholder: 'CRUD.VIEW_OPTION.RBIN.PLACEHOLDER',
            hint: 'CRUD.VIEW_OPTION.RBIN.HINT',
            value: null,
            default: null,
            option: null, // will be filled dynamically at initialization
        },
    ...
    */
    ...
    public readonly _mutationFormError = signal<any>({});
    public readonly mutationFormError = this._mutationFormError.asReadonly();

    public readonly _mutationFormModel = signal<any>({});
    ...
    // signal ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
    private readonly _searchFilterFieldObj = signal<CrudStateSearchFilterFieldObjType>({} as any);
     ...
     */
    private readonly _searchFormLayout = signal<Signal<CrudStateFormFieldObjType[]> | null>(null);
    public readonly searchFormLayout = computed<CrudStateFormFieldObjType[]>(() =>
        this._searchFormLayout()?.() ?? [this.viewOptionFieldObj(), this.searchFilterFieldObj()]
    );

    // form ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
    // need a logic to get from child or auto generate from mutationFieldObj()
    // take reference from: src/app/module/shared/preboarding/signup/state.ts
    private readonly _searchFormError = signal<any>({});
    public readonly searchFormError = this._searchFormError.asReadonly();

    private readonly _searchFormModel = signal<any>({});
    public readonly searchFormModel = this._searchFormModel.asReadonly();

    private readonly _searchFormProcessing = signal(false);
    public readonly searchFormProcessing = this._searchFormProcessing.asReadonly();

    public searchForm = form(this._searchFormModel, (sp) => {
    ...
    public setSearchFormLayout(layout: Signal<CrudStateFormFieldObjType[]>): void {
        this._searchFormLayout.set(layout);
    }
