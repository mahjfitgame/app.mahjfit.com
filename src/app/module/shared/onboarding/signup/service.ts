// file: src/app/module/shared/onboarding/signup/service.ts
...
import { ONBOARDING_SIGNUP_I18N_KEY } from "./const";
...
export class SignupService implements FoundationModuleServiceType {
    ...
    public initI18n(): void {
        this.i18n.useModule(ONBOARDING_SIGNUP_I18N_KEY);
    }
            ...
            try {
                ...
                if (!response.keyid) {
                    const message = response.snapshot?.error?.[0]
                        ...
                        ?? this.i18n.translate('ONBOARDING_SIGNUP.MESSAGE.ERROR');
                    this.notifyBanner.error(message);
                } else {
                    const message = response.snapshot?.success?.[0]
                        ?? response.snapshot?.message?.[0]
                        ?? this.i18n.translate('ONBOARDING_SIGNUP.MESSAGE.SUCCESS');
