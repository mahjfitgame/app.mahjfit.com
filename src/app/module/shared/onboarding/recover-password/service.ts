// file: src/app/module/shared/onboarding/recover-password/service.ts
...
import { ONBOARDING_RECOVERPASSWORD_I18N_KEY } from './const';
...
export class RecoverPasswordService implements FoundationModuleServiceType {
    ...
    public initI18n(): void {
        this.i18n.useModule(ONBOARDING_RECOVERPASSWORD_I18N_KEY);
    }

    public initRouteToken(): void {
        //const token = this.route.getParamPublicid();
        const token = this.state.publicid();
        ...
        const passRecoverToken = this.route.getQueryParamPassRecoverToken();
        if (passRecoverToken) {
        ...
        try {
            ...
            if (!succeeded) {
                const message = response.snapshot?.error?.[0]
                    ...
                    ?? this.i18n.translate('ONBOARDING_RECOVERPASSWORD.MESSAGE.ERROR');
            ...
            const message = response.snapshot?.success?.[0]
                ...
                ?? this.i18n.translate('ONBOARDING_RECOVERPASSWORD.MESSAGE.SUCCESS');
