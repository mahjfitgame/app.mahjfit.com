...
export class GameService {
    ...
    public readonly isLoggedin = computed(() => this.ctxp.state.isAuthenticated());
    // state
    ...
    */
    public async startGame() {
        try {
            ...
            const clientInput: GameCreateInputDto = {
            ...
            if(http.data.length > 0){
                ...
                if(arrResponse.jwt){
                    console.log('JWT', arrResponse.jwt);
                }

                this.api.sdk.graphql.use(GameEngine);
                
                const clientStartInput: GameStatePublicStartInputDto = ({
                    ...arrResponse,
                    // ensure required fields for GameStatePublicStartInputDto
                    owner_u_id: arrResponse.owner_u_id ?? arrResponse.current_turn_u_id ?? 0,
                ...
                console.log('START GAME', startResp);
            }
        }catch (error) {
